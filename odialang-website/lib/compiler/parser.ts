...
export class Parser {
  ...
  private parseStatement(): StatementNode {
    this.skipNewlines();
    if (this.isKeyword("dhara")) return this.parseVariableDeclaration();
    if (this.isKeyword("dekha")) return this.parsePrintStatement();
    if (this.isKeyword("jadi")) return this.parseIfStatement();
    if (this.isKeyword("jebe")) return this.parseWhileStatement();
    if (this.isKeyword("karya")) return this.parseFunctionDeclaration();
    if (this.isKeyword("fera")) return this.parseReturnStatement();
    if (this.isKeyword("aarambha")) return this.parseForStatement();
    return this.parseExpressionStatement();
  }

  private parseVariableDeclaration(): VariableDeclarationNode {
    this.expectKeyword("dhara");
    const nameToken = this.expect(TokenType.IDENTIFIER, "Expected variable name after 'dhara'");
    const identifier: IdentifierNode = { type: "Identifier", name: String(nameToken.value) };
  ...
  private parseIfStatement(): IfStatementNode {
    ...
    while (!this.isAtEnd() && !this.isKeyword("nahele") && !this.isKeyword("sesa")) {
  ...
  private parseForStatement(): ForStatementNode {
    ...
    const iteratorToken = this.expect(TokenType.IDENTIFIER, "Expected iterator variable after 'aarambha'");
    const iterator: IdentifierNode = { type: "Identifier", name: String(iteratorToken.value) };
  ...
  private parseFunctionDeclaration(): FunctionDeclarationNode {
    ...
    const nameToken = this.expect(TokenType.IDENTIFIER, "Expected function name after 'karya'");
    const name: IdentifierNode = { type: "Identifier", name: String(nameToken.value) };
    this.expect(TokenType.LPAREN, "Expected '(' after function name");
    const params: IdentifierNode[] = [];
    if (!this.check(TokenType.RPAREN)) {
      do {
        const paramToken = this.expect(TokenType.IDENTIFIER, "Expected parameter name");
        params.push({ type: "Identifier", name: String(paramToken.value) });
  ...
  private parseReturnStatement(): ReturnStatementNode {
    ...
    if (this.check(TokenType.NEWLINE) || this.check(TokenType.EOF) || this.isKeyword("sesa")) {
  ...
  private parseTerm(): ExpressionNode {
    ...
    while (this.check(TokenType.ARITHMETIC_OPERATOR) && ["+", "-"].includes(String(this.peek().value))) {
      this.advance();
      const operator = String(this.previous().value);
  ...
  private parsePrimary(): ExpressionNode {
    ...
    if (this.match(TokenType.STRING)) {
      return { type: "StringLiteral", value: String(this.previous().value) };
    }
    if (this.match(TokenType.BOOLEAN)) {
      return { type: "BooleanLiteral", value: Boolean(this.previous().value) };
    }
  ...
  private expectKeyword(keyword: string): Token {
    const token = this.peek();
    if (token.type === TokenType.KEYWORD && token.value === keyword) return this.advance();
    throw this.error(token, `Expected keyword '${keyword}'`);
  }

  private expectAssignmentOperator(message: string): Token {
    const token = this.peek();
    if (token.type === TokenType.ASSIGNMENT_OPERATOR && token.value === "=") return this.advance();
  ...
  private error(token: Token, message: string): Error {
    return new Error(`[Parser Error] ${message} at line ${token.line}, column ${token.column}`);
  }
}